...
def train_AE(net, X, Y, lr=0.1, epochs=100, CNN=False, X_out=None, verbose=0):
    if X_out == None:
        X_out = X
...
def train_TAE(X, Y, n_clusters=2, lr=0.1, reg=0, embed=2, epochs=100, number_of_batches=1, linear=True, CNN=False, X_out=None, verbose=0):
    if X_out == None:
        X_out = X
    X = X.to(device)
    Y = Y.to(device)
